export default class HTMLResizeObserverElement extends HTMLElement {
	
	static $observed = Symbol('HTMLResizeObserverElement.observed');
	static $observer = Symbol('HTMLResizeObserverElement.observer');
	static tagName = 'resize-observer';
	...
	static observed(entries, observer) {
		...
		//		style.setProperty(prefix + k + '-raw', v0),
		//		style.setProperty(prefix + k, v0 + 'px'),
		//		isSize &&
	...
	constructor() {
		...
		const { $observed, $observer } = HTMLResizeObserverElement, { constructor: { observed } } = this;
...
customElements.define(HTMLResizeObserverElement.tagName, HTMLResizeObserverElement);